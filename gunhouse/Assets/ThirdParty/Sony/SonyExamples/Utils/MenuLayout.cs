...
public class MenuLayout
{
	...
	public void HandleInput()
	{
		...
		KeyCode PadUpKeyCode = KeyCode.JoystickButton10;
		KeyCode PadDownKeyCode = KeyCode.JoystickButton12;
		
		float MenuEventDeltaTime = Time.timeSinceLevelLoad - timeOfLastChange;
		
		// use simple keycodes to read the pad buttons ... means we can be sure there are no mouse buttons included
		bool advance = Input.GetKey(KeyCode.JoystickButton0);
		bool back = Input.GetKey(KeyCode.JoystickButton1);
		
		if (advance && (MenuEventDeltaTime > repeatTime))   // (X)
		{
			buttonPressed = true;		// inject a button press ... as if it had come from IMGUI
	...
	public bool AddItem(string name, bool enabled = true)
	{
		...
		if (numItems > 0)
		{
			...
			else if (buttonPressed && enabled && selectedItemIndex == currCount) // injecting button presses without using IMGUI events (which only handle mouse events)
			{
				clicked = true;
				buttonPressed = false;
			}
