...
public class SonyNpCloudTUS : IScreen
{
	...
	string m_VirtualUserOnlineID = "_ERGVirtualUser1";
    ...
    {
		m_MenuTus = new MenuLayout(this, 550, 34);
	...
	public void MenuTus(MenuStack menuStack)
    {
        ...
        if (m_MenuTus.AddItem("TUS Set Data", TUSIsReady))
        {
        ...
        if (m_MenuTus.AddItem("TUS Request Data", TUSIsReady))
        {
...
#if (UNITY_5_2 || UNITY_5_3 || UNITY_5_4) && (UNITY_PS4 || UNITY_PSP2)
		if (m_MenuTus.AddItem("TUS Save PlayerPrefs", TUSIsReady))
		{
		...
		if (m_MenuTus.AddItem("TUS Load PlayerPrefs", TUSIsReady))
		{
		...
		if (m_MenuTus.AddItem("TUS Set Variables", TUSIsReady))
		{
		...
		if (m_MenuTus.AddItem("TUS Request Variables", TUSIsReady))
		{
		...
		if (m_MenuTus.AddItem("TUS Set Variables VU", TUSIsReady))
		{
			// Setup an array of variables to set, each variable consists of a slot ID and a value.
			...
			ErrorHandler(Sony.NP.TusTss.SetTusVariablesForVirtualUser(m_VirtualUserOnlineID, variables));
		}

		if (m_MenuTus.AddItem("TUS Request Variables VU", TUSIsReady))
		{
			// Setup up an array of slot IDs for the variables we want to request.
			int[] slotIDs = { 5 };

			// Request the variables.
			ErrorHandler(Sony.NP.TusTss.RequestTusVariablesForVirtualUser(m_VirtualUserOnlineID, slotIDs));
		}

		if (m_MenuTus.AddItem("TUS Modify Variables VU", TUSIsReady))
		{
			// Setup an array of variables to set, each variable consists of a slot ID and a value.
			Sony.NP.TusTss.TusVariable[] variables = new Sony.NP.TusTss.TusVariable[1];
			...
			ErrorHandler(Sony.NP.TusTss.ModifyTusVariablesForVirtualUser(m_VirtualUserOnlineID, variables));
		}
