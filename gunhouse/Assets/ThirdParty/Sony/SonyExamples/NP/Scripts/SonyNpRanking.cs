...
{
	MenuLayout m_RankingMenu;
	ulong m_CurrentScore = (ulong)0 << 32;
	int m_RankBoardID = 0;
	...
	{
		return m_RankingMenu;
	}

	public void Initialize()
	{
		m_RankingMenu = new MenuLayout(this, 450, 34);
	...
	public void MenuRanking(MenuStack menuStack)
	{
		...
		m_RankingMenu.Update();

		if (m_RankingMenu.AddItem("Register Score", rankingAvailable && !Sony.NP.Ranking.RegisterScoreIsBusy()))
		{
			OnScreenLog.Add("Registering score: " + m_CurrentScore);
			ErrorHandler(Sony.NP.Ranking.RegisterScore(m_RankBoardID, m_CurrentScore, "Insert comment here"));
			m_CurrentScore++;
		}

		if (m_RankingMenu.AddItem("Register score & data", rankingAvailable && !Sony.NP.Ranking.RegisterScoreIsBusy()))
		{
			OnScreenLog.Add("Registering score: " + m_CurrentScore);
			byte[] additionaldata = new byte[64];
			for (byte i=0;i<64;i++) { additionaldata[i]=i; }	// fill with dummy data as a test
			
			ErrorHandler(Sony.NP.Ranking.RegisterScoreWithData(m_RankBoardID, m_CurrentScore, "Insert comment here" , additionaldata));
			m_CurrentScore++;
		}
		
		if (m_RankingMenu.AddItem("Own Rank", rankingAvailable && !Sony.NP.Ranking.RefreshOwnRankIsBusy()))
		{
			ErrorHandler(Sony.NP.Ranking.RefreshOwnRank(m_RankBoardID));
		}

		if (m_RankingMenu.AddItem("Friend Rank", rankingAvailable && !Sony.NP.Ranking.RefreshFriendRankIsBusy()))
		{
			ErrorHandler(Sony.NP.Ranking.RefreshFriendRank(m_RankBoardID));
		}

		// repeated calls to this will go through the board
		if (m_RankingMenu.AddItem("Rank List", rankingAvailable && !Sony.NP.Ranking.RefreshRankListIsBusy()))
		{
			...
			ErrorHandler(Sony.NP.Ranking.RefreshRankList(m_RankBoardID, rankRangeStart, rankRangeCount));
		}

		if (m_RankingMenu.AddBackIndex("Back"))
		{
			menuStack.PopMenu();
		}
	}
