...
{
	MenuLayout m_MenuUser;
	string m_RemoteOnlineID = "Q-ZLqkCtBK-GB-EN";
	byte[] m_RemoteNpID = null;
#if UNITY_PS4
	...
	{
#if UNITY_PS4
		m_MenuUser = new MenuLayout(this, 720, 34);
#else
		m_MenuUser = new MenuLayout(this, 560, 34);
#endif
	...
	//public bool UserIsSignedIntoPSN()
	//{
	//	return Sony.NP.User.IsSignedInPSN;
	//}
	...
	public void MenuUser(MenuStack menuStack)
	{
		...
		if (m_MenuUser.AddItem("Get My Profile", !Sony.NP.User.IsUserProfileBusy))
		{
		...
		if (m_MenuUser.AddItem("Get Remote Profile (onlineID)", signedIn && !Sony.NP.User.IsUserProfileBusy))
		{
			...
			//
			Sony.NP.ErrorCode res = Sony.NP.User.RequestRemoteUserProfileForOnlineID(m_RemoteOnlineID);
			if (res != Sony.NP.ErrorCode.NP_OK)
			{
				Sony.NP.ResultCode result = new Sony.NP.ResultCode();
				Sony.NP.User.GetLastUserProfileError(out result);
		...
		if (m_MenuUser.AddItem("Get Remote NpID", signedIn && !Sony.NP.User.IsUserProfileBusy))
		{
			// Lookup another users NpID from their online ID.
			Sony.NP.ErrorCode res = Sony.NP.User.RequestRemoteUserNpID(m_RemoteOnlineID);
			if (res != Sony.NP.ErrorCode.NP_OK)
			{
				Sony.NP.ResultCode result = new Sony.NP.ResultCode();
				Sony.NP.User.GetLastUserProfileError(out result);
		...
		if (m_MenuUser.AddItem("Get Remote Profile (npID)", (m_RemoteNpID != null) && signedIn && !Sony.NP.User.IsUserProfileBusy))
		{
			// Lookup another users profile from their npID.
			Sony.NP.ErrorCode res = Sony.NP.User.RequestRemoteUserProfileForNpID(m_RemoteNpID);
			if (res != Sony.NP.ErrorCode.NP_OK)
			{
				Sony.NP.ResultCode result = new Sony.NP.ResultCode();
				Sony.NP.User.GetLastUserProfileError(out result);
		...
		if (m_MenuUser.AddItem("Get local users status & NPIDs"))
		{
		...
		if (m_MenuUser.AddItem("Use slot 0 for all NPToolkit functions"))
		{
		...
		if (m_MenuUser.AddItem("Use slot 1 for all NPToolkit functions"))
		{
		...
		if (m_MenuUser.AddItem("Use slot 2 for all NPToolkit functions"))
		{
		...
		if (m_MenuUser.AddItem("Use slot 3 for all NPToolkit functions"))
		{
		...
		if (m_MenuUser.AddBackIndex("Back"))
		{
			menuStack.PopMenu();
		}
	}
	...
	{
		m_RemoteNpID = Sony.NP.User.GetCachedRemoteUserNpID();

		string npID = System.Text.Encoding.Default.GetString(m_RemoteNpID);
