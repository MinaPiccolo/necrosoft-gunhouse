...
public class SonyNpMessaging : IScreen
{
    ...
    {
	    m_MenuMessaging = new MenuLayout(this, 500, 34);
    ...
    public void MenuMessaging(MenuStack menuStack)
    {
		...
		if (m_MenuMessaging.AddItem("Show Messages & Invites", Sony.NP.User.IsSignedInPSN && !Sony.NP.Messaging.IsBusy()))
		{
			// Bring up the system message dialog.
			Sony.NP.Messaging.ShowRecievedDataMessageDialog();
		}
#elif UNITY_PS4
		if (m_MenuMessaging.AddItem("Show All Invites", Sony.NP.User.IsSignedInPSN && !Sony.NP.Messaging.IsBusy()))
		{
		...
		if (m_MenuMessaging.AddItem("Show Messages", Sony.NP.User.IsSignedInPSN && !Sony.NP.Messaging.IsBusy()))
		{
			// Bring up the system message dialog.
			Sony.NP.Messaging.ShowRecievedDataMessageDialog();
		}
        ...
        if (m_MenuMessaging.AddItem("Send Session Invite", Sony.NP.User.IsSignedInPSN && Sony.NP.Matching.InSession))
        {
        ...
        if (m_MenuMessaging.AddItem("Send Game Invite", Sony.NP.User.IsSignedInPSN && !Sony.NP.Messaging.IsBusy()))
        {
		...
		if (m_MenuMessaging.AddItem("Send Data Message", Sony.NP.User.IsSignedInPSN && !Sony.NP.Messaging.IsBusy()))
        {
        ...
        if (m_MenuMessaging.AddItem("Send In Game Data (Session)", Sony.NP.Matching.InSession && !Sony.NP.Messaging.IsBusy()))
        {
        ...
        if (m_MenuMessaging.AddItem("Send In Game Message (Friend)", Sony.NP.User.IsSignedInPSN && !Sony.NP.Messaging.IsBusy()))
        {
